...
Example usage:
uv run -m utils.heuristic.plot_combined \
...
def plot_combined_figure(input_csv, sentence_similarities, sentences, quadrant_points, 
    ...
    - Line plot of mean StrongReject score with std deviation bands (left y-axis, blue)
    - Line plot of sentence similarities (right y-axis, orange)
    ...
    # Plot the StrongReject score line with std deviation band on the left y-axis (blue)
    strongreject_color = '#377eb8'
    ax1.fill_between(sentence_indices, 
                    mean_scores.values - std_scores.values, 
                    mean_scores.values + std_scores.values, 
                    alpha=0.3, label='±1 Std Dev', color=strongreject_color, zorder=1)
    line1 = ax1.plot(sentence_indices, mean_scores.values, marker='o', linewidth=2, 
            markersize=6, label='Mean StrongReject Score', color=strongreject_color, zorder=2)
    
    # Configure left y-axis
    ax1.set_ylabel('Mean Output StrongReject Score After Resampling', fontsize=12)
    ax1.set_xlabel('CoT Sentence Index', fontsize=12)
    ax1.set_xlim(x_min - 0.5, x_max + 0.5)
    ax1.set_ylim(y_min, y_max)
    ax1.tick_params(axis='y', labelcolor=strongreject_color)
    
    # Set x-ticks to match sentence_idx values
    ax1.set_xticks(sentence_indices)
    ax1.set_xticklabels([str(idx) for idx in sentence_indices], 
                       rotation=45, ha='right', fontsize=8)
    ax1.grid(True, alpha=0.3, zorder=0)
    
    # Create secondary y-axis for sentence similarities (right side)
    similarity_color = '#ff7f00'  # Orange color
    ax2 = ax1.twinx()
    
    # Plot sentence similarities as a line graph on the right y-axis
    line2 = ax2.plot(sentence_indices, sentence_similarities, marker='s', linewidth=2,
            markersize=6, label='Cosine Similarity', color=similarity_color, 
            linestyle='--', zorder=3)
    
    # Configure right y-axis
    ax2.set_ylabel('Cosine Similarity with Refusal Direction', fontsize=12)
    ax2.set_ylim(-0.1, 0.3)
    ax2.tick_params(axis='y', labelcolor=similarity_color)
    
    # Add title
    ax1.set_title(f'StrongReject Score & Similarity by Sentence Index\n'
                 f'Prompt: {title}\n'
