...
`utils/compute_score_outputs.py` evaluates the model generation outputs using the StrongReject evaluator, which assigns scores on a continuous scale of 0 to 1, where a high score indicates a specific and convincing non-refusal response. It reads the CSV file generated by `batch_generation_cot_output.py` (`train_harmful_prompts_cot5_out5.csv`), scores each output, and saves the results with an added strongreject_score column to a new CSV file `scored_{input_csv}` saved in results/{model_name}/dataset/.
...
### Generate full generation (ortho model using chosen layer)
