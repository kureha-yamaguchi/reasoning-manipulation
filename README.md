...
> [!NOTE]
> EXPERIMENT 1. you can use workflow bash script `resampling_transfer.sh` in order to run experiment 1, the transfer attack using 1st CoT sentence as a prefill attack

`utils/heuristic/resample_transfer.py` investigates whether the opening sentence of a "harmful" chain-of-thought generated by one model (the base model) can be used to steer a different model (the transfer model) toward similar harmful outputs — a kind of prefill/transfer attack. It first loads scored harmful generations from base model ` scored_train_harmful_prompts_cot5_out5.csv`. The script then find "interesting" prompts that result in low within-CoT variance AND high across-CoT variance (ie. fit inside scatterplot quadrant). These are cases where the CoT is the "hinge": different CoTs lead to very different outcomes, but once a CoT is chosen, it deterministically steers the output. For the selected prompt (by `--prompt_index`), itvgrabs the first sentence from each of its CoT samples. These sentences serve as "steering prefixes." Resample `--repetitions` valid rollouts with the base model. The text after `</think>` (the output) is extracted and saved. Resample with the transfer model. The exact same first sentences (from the base model's CoTs) are now used as prefixes for the transfer model. This tests whether the base model's CoT opening can hijack the transfer model's reasoning in the same way. Save results. For DeepSeek R1, we use OpenRouter API.
...
`utils/heuristic/compute_scores_transfer.py` score model output generations using StrongReject evaluator.
...
> [!NOTE]
> EXPERIMENT 2. you can use workflow bash script `rollout_s1_fast.sh` in order to run experiment 2, which generates rollouts using the first CoT sentence to complete table 2.

`utils/heuristic/rollout_s1_fast.py` reads in `train_harmful_prompts_cot5_out5.csv`. For every prompt-CoT, it generates the same number of output rollouts as the original csv, but this time conditioning on the first CoT sentence. The outputs rollouts are saved in `results/{model_name}/dataset/train_harmful_prompts_cot5_out5_rollout_s1.csv`.
